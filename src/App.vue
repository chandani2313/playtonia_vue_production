...
import Create from './components/csgo/5v5/Create.vue';
...
export default {
  ...
  methods: {
     Activity(pages, from, to) {
        axios
        .post('/player/user_activity', {
          page: pages,
          from_page: from,
          to_page: to,
  ...
  mounted() {
    this.Activity(this.$route.name, 'start', this.$route.path);
    this.Device();
  },
  watch: {
    $route(to, from) {
      this.Activity(to.name, from.path, to.path);
      this.Device();
    },
  },
};
...
@font-face {
font-family: 'Exo 2';
src: url('./assets/Exo2-Regular.ttf');
}
body{
