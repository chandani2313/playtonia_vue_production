      ...
      <div class="modal__wrapper">
            <div class="table__wrapper animated zoomIn">
                <div class="table__header">
                    <div class="backdrop">
                    <div class="heading">GAME SERVER IP</div>
                ...
                <div class="table__body">
                    <form novalidate>
                        <div class="input-wrapper">
                            <input type="text" v-model="server_ip" required  placeholder="" />
                            <span @click="toggleip()" id="server_ip" toggle="#password-field" class="fa fa-fw fa-eye field-icon toggle-password"></span>
                        </div>
                        <div class="input-wrapper" >
                            <input type="hidden" id="testing-code" :value="static_ip"  />
                        </div>
                        <button class="btn" @click.prevent="copyTestingCode()">COPY IP ADDRESS</button>
                    </form>
                </div>
            </div>
      </div>
...
export default {
  ...
  data() {
    return {
        server_ip: '*********',
        show_ip: false,
        static_ip: this.game.game_server.server_ip.concat(':').concat(this.game.game_server.port),
    };
  },
  props: ['game'],
  methods: {
        ...
        toggleip() {
            if (this.show_ip === false) {
                this.server_ip = this.game.game_server.server_ip.concat(':').concat(this.game.game_server.port);
                this.show_ip = true;
                document.querySelector('#server_ip').classList.remove('fa-eye');
                document.querySelector('#server_ip').classList.add('fa-eye-slash');
            } else {
                this.server_ip = '******';
                this.show_ip = false;
                document.querySelector('#server_ip').classList.remove('fa-eye-slash');
                document.querySelector('#server_ip').classList.add('fa-eye');
            }
        },
        copyTestingCode() {
