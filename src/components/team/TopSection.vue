<template>
  <div class="team">
    <div class="top_wraper"  v-if="team_data">
        <div id="profile-upper">
            ...
            <div id="profile-d">
                <div id="profile-pic">
                    ...
                    <!-- <div id="add-profilepic"><i class="fas fa-camera"></i></div> -->
                </div>
                <div id="u-name" >
                    <div >
                        <div class="profile_name_conent">
                            <div class="profile_name__game">{{team_name}}</div>
                            <div class="profile_name__user">{{team_data.game}}</div>
                    ...
                    <div class="tb" id="m-btns">
                        <div class="td"  v-if="captain">
                            <div class="m-btn" @click="toggle()"><i class="fas fa-user-plus"></i><span>Invite Player</span></div>
                        </div>
                        <div class="td"  v-if="captain">
                            <router-link class="m-btn"  tag="div" :to="{ name: 'EditTeam' ,params: {alias: team_data.team_alias}}"  active-class="active" ><i class="fas fa-cog"></i><span>Team Settings</span></router-link>
                        </div>
                        <div class="td" v-if="!captain && !requested && !join_sent && !is_member || show_join && !join_sent">
                            <div class="m-btn" @click="JoinTeam()"><i class="fas fa-user-plus"></i><span>Join Team</span></div>
                        </div>
                        <div class="td" v-if="!captain && requested && !join_sent">
                            <div class="m-btn" ><i class="fas fa-user-plus"></i><span>Requested</span></div>
                        </div>
                        <div class="td" v-if="!captain && join_sent">
                            <div class="m-btn" ><i class="fas fa-user-plus"></i><span>Requested</span></div>
                        </div>
                         <div class="td" v-if="!captain && is_member && !show_join">
                            <div class="m-btn" @click="LeaveTeam()" ><i class="fas fa-sign-out-alt"></i><span>Leave Team</span></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- <div id="add-coverpic"><i class="fas fa-camera"></i></div> -->
        </div>
    ...
    <Mlbb v-if="team_data && game_id && team_data.game === 'MLBB'" @close="Close()" @join="JoinTeam()" />
    <Csgo v-if="team_data && game_id && team_data.game === 'CSGO'" :error="error" @close="Close()"  />
  </div>
</template>
<script>
// @ is an alias to /src
...
export default {
  ...
  props: ['team_name', 'captain', 'team_id', 'requested', 'is_member'],
  components: {
    Mlbb,
    Csgo,
  },
  ...
  methods: {
    ...
    LeaveTeam() {
      this.$dialog
      .confirm('Confirm To Leave From The Team.')
      .then(() => {
        axios
      .post('/team/leave_team', {
      ...
      .then((response) => {
          if (response.status === 200) {
              ...
              this.$notify({
                    title: 'Success.',
                    group: 'team_notification',
                    text: 'Successfully Left From Team.',
                });
          }
      })
      .catch((e) => {
      ...
      .catch((e) => {
          if (e.response.status === 400) {
            const [ResponseData] = e.response.data.response;
              if (ResponseData === 'request_exists') {
                this.$notify({
                    type: 'warn',
                    title: 'Warning',
                    group: 'team_notification',
                    text: 'Join Request Sent Already.',
                });
              }
              if (ResponseData === 'game_id') {
                  this.game_id = true;
              }
              if (ResponseData === 'part_of_team') {
                this.$notify({
                    type: 'warn',
                    title: 'Warning',
                    group: 'team_notification',
                    text: 'You Are Already Member Of A Team.',
                });
              }
          }
      });
