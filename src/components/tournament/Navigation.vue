<template>
  <div class="navigation">
          <div class="mob_wrapper_outer">
            <span class="mob_arrow" @click="Prev()"><i class="fas fa-chevron-left"></i></span>
            <div class="mob_wrapper">
              <ul class="mob_nav" >
                <router-link data-animation="ripple" class="mob_navli" tag="li" :to="{ name: menu_name ,params: {id: alias}}"  active-class="active" exact><div class="mob_nav_title">{{title}}</div></router-link>
              </ul>
            </div>
            <span class="mob_arrow" @click="Next()"><i class="fas fa-chevron-right"></i></span>
          </div>
  </div>
</template>
<script>
...
export default {
  ...
  props: ['menus', 'total', 'alias'],
  methods: {
      Next() {
          if (this.current_page < this.total) {
            this.current_page += 1;
            const [Result] = this.menus.filter((c) => c.order === this.current_page);
            this.menu_name = Result.name;
            this.title = Result.title;
            this.$router.push({ name: Result.name, params: { id: this.alias } });
          }
      },
      Current() {
          if (this.current_page < this.total) {
            const [Result] = this.menus.filter((c) => c.name === this.$router.history.current.name);
            this.menu_name = Result.name;
            this.title = Result.title;
            this.current_page = Result.order;
